...
- 接受 Builder Graph payload 中的 rag 定義，並轉成 `rb_rag_supplement` 可落庫的結構
...
Mapping 規則：
- `ragType` → `rb_rag_supplement.rag_type`
- `title` → `rb_rag_supplement.title`
- `content` → `rb_rag_supplement.content`
- `orderNo` → `rb_rag_supplement.order_no`
- `overridable` → `rb_rag_supplement.overridable`
- `retrievalMode` → `rb_rag_supplement.retrieval_mode`

若前端送的是精簡 payload，後端可補預設值：
- `overridable` 未傳 → `false`
- `retrievalMode` 未傳 → `"full_context"`
- `title` 未傳 → 可由後端用 `ragType + orderNo` 自動補一個可讀名稱
...
#### 紅框 text 的現階段定義
- 初期不把畫布中的紅框 `text` 做成獨立資料表節點
- 畫布若要表達「這裡的預設內容可以被使用者輸入覆蓋」，請以 `overridable=true` 的 RAG 來表示
- 也就是說，前端 text 目前仍是 consult request 的輸入，不是 graph 裡的一筆永久資料
